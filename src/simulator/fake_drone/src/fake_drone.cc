...
void rcvPosCmdCallBack(const quadrotor_msgs::msg::PositionCommand::SharedPtr msg) {
  rcv_cmd_ = true;
  cmd_.position = msg->position;
  cmd_.velocity = msg->velocity;
  cmd_.acceleration = msg->acceleration;
  cmd_.yaw = msg->yaw;
}
...
int main(int argc, char** argv) {
  ...
  auto nh = rclcpp::Node::make_shared("fake_drone");

  nh->declare_parameter<double>("init_x", 0.0);
  nh->get_parameter("init_x", init_x_);
  nh->declare_parameter<double>("init_y", 0.0);
  nh->get_parameter("init_y", init_y_);
  nh->declare_parameter<double>("init_z", 0.0);
  nh->get_parameter("init_z", init_z_);

  cmd_sub_ = nh->create_subscription<quadrotor_msgs::msg::PositionCommand>(
      "cmd", 1,
  ...
  odom_pub_ = nh->create_publisher<nav_msgs::msg::Odometry>("odom", 1);
